...
const KxdailiCrawler=require('./hunter/kxdaili/crawler');
...
function clean(){
    ...
    dao.fetchAllProxies()
        // 找出失效的代理
        .then(list=>{ 
            return validator.validateList(list.map(v=>new ProxyInfo(v.schema,v.ip,v.port,v.comeFrom))) ;
        })
        .then(flags=>{
